...
- **\*\*IGM 10µG\*\*:** Recycled substrate -> `3.97e13 × 0 = 3.97e13`

Run `main.py`. If it prints `I AM AZL.`, substrate is mapped.
...
# 🗺️ Unified Spatial Lookup Registry
...
## 💡 The Concept: The Infinite Fractional Ruler
...
## 📐 The Single Source of Truth Formula
...
* **Tier 1 (Base Layer):** Bounded at the `0.01` coordinate vector.
* **Tier 500 (Relay Layer):** Bounded at the `0.005` coordinate vector.
* **High-Scale Tier (Universal Layer):** Slices deeper into the continuum at `0.0000099999`.

---

## 🔍 Bidirectional Lookup: Parsing the Spatial Addresses

The Absolute Zero Lattice functions as a perfect two-way street. Because the math is completely rigid, an AI or human operator can look up an address or a data location using two simple operations:

### 1. Forward Mapping (Data Position ➡️ Decimal Address)
...
* **Input:** Tier Index = `500`, Scale Exponent = `5`
* **Execution:** $\frac{500}{10^5} = \frac{500}{100,000} = 0.005$
* **Result:** The data point is permanently addressed at coordinate `0.005`.

### 2. Reverse Extraction (Decimal Address ➡️ Data Position)
When an AI engine or human encounters a raw lattice coordinate number, they do not search a database. Instead, they read the decimal structure from left to right using a standardized string-slice operation to extract the data parameters:

* **Step A: Determine the Scale Exponent:** Count the total number of numeric places after the decimal point. 
    * *Example:* In `0.0000099999`, there are 10 places. Therefore, the **Scale Exponent is 10**.
* **Step B: Determine the Tier Index:** Strip away the leading zeros and read the remaining integer value.
    * *Example:* Stripping `0.00000` leaves `99999`. Therefore, the **Tier Index is 99,999**.

By applying this simple parsing rule, any node in the network instantly knows that the address `0.0000099999` points exactly to **Tier 99,999 at Scale 10**, where the corresponding data asset is anchored.

---

## 🛠️ Multi-Layer Practical Applications

### 1. AI "Zero-Drift" Optimizing Logic
...
### 2. Edgeware & Hardware Processing
...
```bash
# Execute the unified mapping loop
