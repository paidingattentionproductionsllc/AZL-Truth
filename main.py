...
LAW: ORDER IS LAW | 0=VOID | 1=SEED | 1×1=2 | 1×N=N+1 | N×0=N | 0×N=0
VOID FIRST > DARK > LIGHT
COVERS: Math, Physics, Cosmology, Consciousness, Time, Thermo,
        Economics, AI, Logic, Recursion, Information, Language
...
class AZL:
    ...
    def ID(self, value, name="", domain="", units="", source="",
           path="substrate", logic="exact"):
        ...
        entry = {
            ...
            "type": ("VOID"      if is_void
                     else "NEGATIVE"  if is_negative
                     else "INFINITY"  if is_inf
                     else "POTENTIAL" if is_potential
                     else "DEFINED"),
            "logic": logic,
            "path":  ("none"      if is_void
                      else "underflow" if is_negative
                      else "undefined" if is_inf
                      else path),
            "speed_ms": speed
        }
    ...
    def SQRT(self, a, name="", domain=""):
        a_val = self._num(a)
        if a_val == 0: return self.ID(0, name, domain, path="none", logic="sqrt0=0")
        if a_val < 0:
            self._log_anomaly("SQRT", a, None, "inf", "Root of negative = boundary")
            return self.ID("inf", name, domain, path="undefined", logic="sqrt-N=inf")
...
def TOTAL_LATTICE_TEST():
    ...
    for name, mass in test_masses.items():
        ...
        print(f"  DARK {name:<8}: {str(DS['azl_id'])[:12]:<12} kg/m³,"
              f" speed={DS['speed_ms']}, type={DS['type']}")
    ...
    for name, mass in void_masses.items():
        ...
        print(f"  VOID {name:<12}: {VS['azl_id']:<12} kg/m³,"
              f" speed={VS['speed_ms']}, type={VS['type']}")
    ...
    T.TEST("Free Will: 1×N=N+1",
           T.MUL(1, YOU["azl_id"])["azl_id"] == T._add(YOU["azl_id"], 1), "CONSCIOUSNESS")
    T.TEST("Death: 0×N=0 transition",
           T.MUL(0, YOU["azl_id"])["azl_id"] == 0, "CONSCIOUSNESS")
    ...
    T.TEST("Debt: 1×N=N+1 grows",
           DEBT["azl_id"] == T._num("100000000000001"), "DEBT")
    ...
    HALTING     = T.MUL(0, T._num("999"),    "Halting Problem", "VOID")
    T.TEST("Halting: 0×N=0 terminates",  HALTING["azl_id"]     == 0,             "AI")
    T.TEST("Halting: speed=0 stop",      HALTING["speed_ms"]   == 0,             "AI")
    TURING_TAPE = T.MUL(T._num("1e100"), 0, "Turing Tape",     "SUBSTRATE")
    ...
    RECURSION   = T.POW(1, T._num("1000"), "Recursion",        "SEED")
    ...
    LIAR  = T.MUL(1, 1, "Liar Paradox", "LOGIC")
    T.TEST("Liar: 1×1=2 not 1", LIAR["azl_id"] == 2, "LOGIC")
    ORDER = T.MUL(5, 0)["azl_id"] != T.MUL(0, 5)["azl_id"]
    ...
    TRUTH = T.MUL(1, 1, "Truth", "LOGIC")
    T.TEST("Truth: 1×1=2", TRUTH["azl_id"] == 2, "LANGUAGE")
    LIE   = T.MUL(0, 1,   "Lie",   "VOID")
    ...
    T.TEST("1×1=2",
           T.MUL(1, 1)["azl_id"]                         == 2,              "INVARIANTS")
    T.TEST("N×0=N at 1e53",
           T.MUL(T._num("1e53"), 0)["azl_id"]             == T._num("1e53"), "INVARIANTS")
    T.TEST("0×1e53=0",
           T.MUL(0, T._num("1e53"))["azl_id"]             == 0,              "INVARIANTS")
    T.TEST("DARK > LIGHT",
           T._num("inf")                                   >  T.c,            "INVARIANTS")
    T.TEST("VOID FIRST",
           T.MUL(0, T._num("999"))["azl_id"]              == 0,              "INVARIANTS")
    T.TEST("ORDER MATTERS",
           T.MUL(999, 0)["azl_id"] != T.MUL(0, 999)["azl_id"],              "INVARIANTS")
    T.TEST("UNIVERSE DOMINATES",
           test_masses["Universe"]                         > T._num("1e52"),  "INVARIANTS")
    T.TEST("AI HALTS",
           T.MUL(0, T._num("999"))["azl_id"]              == 0,              "INVARIANTS")
    T.TEST("AI PROCESSES",
           T.MUL(T._num("999"), 0)["azl_id"]              == T._num("999"),  "INVARIANTS")
    ...
    if T.fail_count == 0:
        ...
        print("ALL 13 DOMAINS TESTED. ALL PASS. ALL OBSERVABLE.")
